<x-app-layout>
    ...
    <div>
        <x-panel>
            ...
            <x-slot name="body">
                <form method="POST" action="{{ $action == 'create'? route('storeLeave'): route('updateLeave')}}" class="text-black w-full" data-turbo-frame="_top" enctype="multipart/form-data"  onsubmit="return confirm('Are you sure you want to submit this leave application?');">
                 @csrf
                 @if($action == 'edit')
                    @method('PUT')
                                    ...
                                    <x-select onChange="getLeaveBalance()" id="leaveType" name="leaveType">
                                        ...
                                        @if($leaveTypes != null && count($leaveTypes) > 0)
                                            @foreach($leaveTypes as $type)
                                                <option value="{{$type->Code}}" {{$action == 'edit'? $requisition->Leave_Type == $type->Code? 'selected':'' :old('leaveType') }}>{{$type->Code}}</option>
                                            @endforeach
                                        @endif
                                    </x-select>
                                </x-slot>
                        ...
                        <x-grid>
                            <x-grid-col class="hidden leaveHide" id="divStartDate">
                                <x-form-group>
                                    ...
                                    <x-slot name="value">
                                        <x-input type="date" name="startDate" :required="false" id="startDate" onChange="getLeaveDates()" value="{{$action == 'edit' && old('startDate') == null? $requisition->Start_Date:old('startDate')}}" />
                                    </x-slot>
                                </x-form-group>
                            </x-grid-col>
                            ...
                            <x-grid-col class="hidden leaveHide" id="divEndDate">
                                <x-form-group>
                                    ...
                                    <x-slot name="value">
                                        <x-input type="date" name="endDate" :required="false" id="endDate" onChange="getLeaveDates()" value="{{$action == 'edit' && old('endDate') == null? $requisition->End_Date:old('endDate')}}" />
                                    </x-slot>
                                </x-form-group>
                            </x-grid-col>
                                        ...
                                        <x-select name="reliever" id="reliever" class="tom-selects" placeholder="select">
                                            ...
                                            @if($relievers != null)
                                                @foreach($relievers as $reliever)
                                                    <option value="{{$reliever->No}}" {{$action == 'edit'? $requisition->Reliever_No == $reliever->No? 'selected':'' :old('reliever') }}>{{$reliever['No'].' - '.$reliever['First_Name'].' '.$reliever['Middle_Name'].' '.$reliever['Last_Name']}}</option>
                                                @endforeach
                                            @endif
                                        </x-select>
                                    </x-slot>
                            ...
                            <x-grid-col class="sm:grid-cols-1">
                                <x-form-group>
                                    ...
                                    <x-slot name="value">
                                        <x-textarea name="reason">{{$action == 'edit' && old('reason') == null? $requisition->Purpose:old('reason')}}</x-textarea>
                                    </x-slot>
                                </x-form-group>
                            </x-grid-col>
                            <x-grid-col class="sm:grid-cols-1">
                                <x-form-group>
                                    ...
                                    <x-slot name="value">
                                        <x-select name="requestLeaveAllowance">
                                            <option value="0" {{$action == 'edit'? $requisition->Request_Leave_Allowance == false? 'selected':'' :old('requestLeaveAllowance')}}>No</option>
                                            <option value="1" {{$action == 'edit'? $requisition->Request_Leave_Allowance == true? 'selected':'' :old('requestLeaveAllowance') }}>Yes</option>
                                        </x-select>
                                    </x-slot>
                                </x-form-group>
                            </x-grid-col>
...
{{-- <script src="C:\Portals\Staff Portal\node_modules\nice-select2\dist\js\nice-select2.js"></script> --}}
    <script>
        ...
        if(action == 'edit'){
            appliedDays = "<?php echo isset($requisition)? $requisition->Days_Applied:'';?>";
        }
        function fnOnLoad(){
            if(action == 'edit'){
                getLeaveBalance();
        ...
        function getLeaveBalance(){
            ...
            for (let element of leaveHide){element.classList.add('hidden');}
            if(elLeaveType.value != ''){
                loader.classList.remove('hidden');
                axios.get('/staff/leave/balance/'+elLeaveType.value).then(response =>{
                    var leaveDays = response.data.balance;
                    pendingCount = response.data.pendingCount;
                    isHourly = response.data.isHourly;
                    ...
                    if(leaveDays > 0){
                        ...
                        if(pendingCount == 0){
                            ...
                            alert('You cannot apply a new leave while there is another one of the same type that is pending approval.');
                        }
                    }
        ...
        function getLeaveDates(){
            ...
            if(isHourly && appliedDuration > 4){
                ...
                axios.get('/staff/leave/dates/'+elLeaveType.value+'/'+ending.replace('/','_')+'/'+starting.replace('/','_')).then(response =>{
