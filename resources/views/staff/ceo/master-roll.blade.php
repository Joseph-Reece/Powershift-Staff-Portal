<x-app-layout>
    ...
    <div>
        <x-panel maxWidth="max-w-lg">
            <x-slot name="title">Master Roll Report</x-slot>
            <x-slot name="body">
                <form method="POST" action="{{route('generateMasterRollReport')}}" class="text-black w-full" data-turbo-frame="_top" target="_blank">
                 @csrf
                    <x-grid cols="sm:grid-cols-1">
                        ...
                        <x-grid-col>
                            <x-form-group>
                                <x-slot name="label">Posting Group (Optional)</x-slot>
                                <x-slot name="value">
                                    <x-select id="postingGroup" name="postingGroup" :required="false">
                                        <option value="">--select--</option>
                                        @if($PostGroups != null && count($PostGroups) > 0)
                                            @foreach($PostGroups as $PostGroup)
                                                <option value="{{$PostGroup->Code}}" {{old('postingGroup') == $PostGroup->Code? 'selected':''}}>{{$PostGroup->Description}}</option>
                                            @endforeach
                                        @endif
                                    </x-select>
                                </x-slot>
